...
//another way to write it #define RCC_BASE (Peripherals_BASE + 0x00021000UL)
...
//-*-*-*-*-*-*-*-*-*-*-*
// Offset of each register included or taken into account in the struct e.g. CRL 0x0 .. CRH 0x4
typedef struct
{
	volatile uint32_t CRL; // first 4 bytes reserved for CRL register, that indicates directly  offset 0x0
	volatile uint32_t CRH; // 2nd 4 bytes for CRH that indicates directly offset 0x4
...
//-*-*-*-*-*-*-*-*-*-*-*
// same like GPIO register struct
typedef struct
{
...
//another way to do it like  GPIO_t *GPIOA = (GPIO_t *)GPIOA_BASE
